package whitetac.spigotplugin.mcdoge125coin;
...
public class App extends JavaPlugin
{
    ...
    public void onEnable() {
        getLogger().info("Enable Mc doge125coin trader by whitetac");
        if (!setupEconomy() ) {
        ...
        try{
            coinPrice = config.getDouble("doge125");
        }catch(Exception e){
            config.addDefault("doge125", (double) 8000.0);
            saveConfig();
        }
        try{
    ...
    public void onDisable() {
        getLogger().info("Disable Mc doge125coin trader by whitetac");
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String commandLabel, String[] args) {
        ...
        if(lastRefreshTime+500*1<System.currentTimeMillis()) {
            ...
            if(coinPrice<=0){
                ...
                config.set("doge125", 500);
                for (String key : config.getKeys(true)){
                    if(key.equals("btc") || key.equals("doge")|| key.equals("doge125") || key.equals("lastRefreshTime")) continue;
        ...
        if(command.getLabel().equals("doge125")) {
            ...
            sender.sendMessage("------------------------------");
            sender.sendMessage(String.format("125배 (-0.8%%부터 청산) 도지코인 시세정보는 업비트api 기반으로 이루어집니다. (최소 수량 0.001)"));
            sender.sendMessage(String.format("거래방법 /doge125 buy|sell 수량"));
            sender.sendMessage(String.format(" "));
            sender.sendMessage(String.format("시세 doge125/MD %s", coinPrice));
            sender.sendMessage(String.format("보유 코인 %sdoge125", Math.round(coinAmount*1000.0)/1000.0));
            sender.sendMessage(String.format(" "));
            sender.sendMessage(String.format("구매가능 수량 %sdoge125 (%s)", Math.floor(moneyAmount/coinPrice*1000.0)/1000.0, econ.format(moneyAmount)));
            if(avgPrice>0){
    ...
    public void getPrice() {
        ...
        try {
            ...
            if (responseCode == 400) {
                ...
                coinPrice = lastPrice;
                try{
                    config.getDouble("doge125");
                }catch(Exception e){
                    config.set("doge125", 500);
                    config.set("doge", coinPrice);
                }
                config.set("doge125", config.getDouble("doge125")*(1+ ((coinPrice/config.getDouble("doge")-1)*125) ));
                config.set("doge", coinPrice);
                coinPrice = config.getDouble("doge125");
