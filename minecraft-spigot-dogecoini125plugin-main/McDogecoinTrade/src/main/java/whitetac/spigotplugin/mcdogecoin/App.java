package whitetac.spigotplugin.mcdogei125coin;
...
public class App extends JavaPlugin
{
    ...
    public void onEnable() {
        getLogger().info("Enable Mc dogei125coin trader by whitetac");
        if (!setupEconomy() ) {
        ...
        try{
            coinPrice = config.getDouble("dogei125");
        }catch(Exception e){
            config.addDefault("dogei125", (double) 8000.0);
            saveConfig();
        }
        try{
    ...
    public void onDisable() {
        getLogger().info("Disable Mc dogei125coin trader by whitetac");
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String commandLabel, String[] args) {
        ...
        if(lastRefreshTime+500*1<System.currentTimeMillis()) {
            ...
            if(coinPrice<=0){
                ...
                config.set("dogei125", 500);
                for (String key : config.getKeys(true)){
                    if(key.equals("btc") || key.equals("dogei")|| key.equals("dogei125") || key.equals("lastRefreshTime")) continue;
        ...
        for (String key : config.getKeys(true)){
            if(key.equals("btc") || key.equals("dogei")|| key.equals("dogei125") || key.equals("lastRefreshTime")) continue;
            //sender.sendMessage(key);
            if(coinPrice <= Double.parseDouble(config.getString(key).split(",")[1])*(1-0.008) ){
                config.set(uuid, "0,0");
            }
        }
        ...
        if(command.getLabel().equals("dogei125")) {
            ...
            sender.sendMessage("------------------------------");
            sender.sendMessage(String.format("125배 인버스 (-0.8%%부터 청산) 도지코인 시세정보는 업비트api 기반으로 이루어집니다. (최소 수량 0.001)"));
            sender.sendMessage(String.format("거래방법 /dogei125 buy|sell 수량"));
            sender.sendMessage(String.format(" "));
            sender.sendMessage(String.format("시세 dogei125/MD %s", coinPrice));
            sender.sendMessage(String.format("보유 코인 %sdogei125", Math.round(coinAmount*1000.0)/1000.0));
            sender.sendMessage(String.format(" "));
            sender.sendMessage(String.format("구매가능 수량 %sdogei125 (%s)", Math.floor(moneyAmount/coinPrice*1000.0)/1000.0, econ.format(moneyAmount)));
            if(avgPrice>0){
    ...
    public void getPrice() {
        ...
        try {
            ...
            if (responseCode == 400) {
                ...
                coinPrice = lastPrice;
                try{
                    if(config.getDouble("dogei125") == null){
                        config.set("dogei125", 500);
                ...
                config.set("dogei125", config.getDouble("dogei125")*(1+ ((coinPrice/config.getDouble("dogei")-1)*-125) ));
                config.set("dogei", coinPrice);
                coinPrice = config.getDouble("dogei125");
